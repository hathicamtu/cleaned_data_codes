...
(majority) backward elimination: > 50% occuring in the final models
...
expr <- expression(f1 <- glm(late_culture_conversion ~ MTB_load + hb + ptretx_bmi + qol_usual_activity + bl_age +  prettx_prevtbtx + resistance_pattern, 
                             family = binomial),
                   f2 <- step(f1, scope = scope, direction = "backward"))
...
## 5.3. check multicollinearity with GVIF
```{r}
...
roc_list <- lapply(1:30, function(i) {
  # Fit the model for each imputation dataset
  fit <- glm(late_culture_conversion~ bl_age+ hb+ MTB_load+ prettx_prevtbtx +ptretx_bmi+  qol_usual_activity, 
             family = binomial, 
  ...
  # Calculate ROC for each imputation
  roc_obj <- roc(complete(mids_3rd, i)$late_culture_conversion, predicted_probs)
  return(roc_obj)
})

# Plot the ROC curves using plot.roc function
plot.roc(roc_list[[1]], col = "blue", main = "ROC Curves", 
         print.auc = F, legacy.axes = T) 

class(roc_list[[1]])
# Add the ROC curves for the other imputed datasets
for (i in 2:30) {
  plot.roc(roc_list[[i]], col = rainbow(10)[i], add = TRUE, print.auc = F)
}
...
```{r}
internal_validate_3 <- psfmi_validate(
  pobj = pool_D1_model_3rd,
  val_method = "MI_boot",
  miceImp = T,
  int_val = T,
  nboot = 200, # the number of bootstrap resamples
  plot.method = "mean",
  cal.plot = T,
  groups_cal = 7)
...
on the hierarchy of the regression coefficients in the final model 
(each coefficient was divided by the smallest coefficient (bl_age =0.60) and rounded 
to the nearest integer). The score’s predictive performance (AUC) 
was assessed and compared with that of the original model. Mortality 
risk corresponding to each score was calculated
...
summary(pool(fit_pool_3_final), conf.int=T)
tbl_regression(fit_pool_3_final,exponentiate = TRUE)
#vif(fit_pool_3_final$analyses[[1]])
```
...
imp1_d2_long_not_include_cat_3_red_score %>% 
  select(bl_age, hb, contains("MTB"), ptretx_bmi , prettx_prevtbtx, qol_usual_activity, late_culture_conversion, TotalScore)

table(imp1_d2_long_not_include_cat_3_red_score$TotalScore)

imp1_d2_long_not_include_cat_3_red_score %>% 
  filter(TotalScore==2) %>% 
  filter(late_culture_conversion==1) %>% 
  distinct(.id)
```

calculate risk scores
for stacked 30 imputed datasets (equal contribution of imputed dataset)
...
cat("✅ Mean optimal cutoff (Youden):", round(mean_cutoff, 3), "\n")
cat("📈 Mean Youden Index:", round(mean_youden, 3), "\n")
...
```{r}
# 1. Get predicted probabilities per imputation
...
# 2. Get the average prediction per individual (pooled prediction)
pooled_preds <- rowMeans(pred_matrix)

# 3. Use the observed outcome from any imputed dataset (e.g., first one)
true_outcome <- complete(mids_3rd_score, 1)$late_culture_conversion

# 4. Calculate ROC with pooled predicted probabilities
pooled_roc <- roc(true_outcome, pooled_preds, direction = "<")

# 5. Get optimal threshold using Youden Index
...
# 6. View results
print(youden_result)
cat("📌 Optimal cutoff (pooled):", round(youden_result$threshold, 3), "\n")
cat("📈 Youden Index:", round(youden_result$youden, 3), "\n")
